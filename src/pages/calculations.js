...
import { Nav, NavbarContainer, NavLogo, LogoIcon, MobileIcon } from '../components/Navbar/NavbarElements';
import { FaBars } from "react-icons/fa";
import { FcEngineering } from "react-icons/fc";
import backgroundImage from '../images/25662.jpg';
...
const StyledTableContainer = styled.div`
  ...
  margin-bottom: 20px; /* Add margin to create space between table and images */
`;
const Calculations = () => {
...
const calculateSphericalRadius = (r) => {
  ...
  return sphericalRadius.toFixed(2); // Limiting to 2 decimal places
};

const calculateNeckRadius = (r) => {
  const neckRadius = 0.17 * 2 * r;
  return neckRadius.toFixed(2); // Limiting to 2 decimal places
};

const calculateHeadHeight = (r) => {
  const headHeight = (2 * r) / 4;
  const s= headHeight.toFixed(2);
  return s; // Limiting to 2 decimal places
};
...
return (
  <>
    ...
    <Container >
      <Title style={{ marginTop: '20px' }}>Design your pressure vessel</Title>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', maxWidth: '800px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', marginRight: '20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', marginTop: '20px' }}>
            <select style={{ height: '40px', width: '100%', borderRadius: '4px', border: '1px solid #ccc', padding: '0 10px', color: '#333', backgroundColor: '#fff' }} onChange={handleDropdownChange}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: '20px', marginBottom: '20px' }}>
              <button onClick={handleClear} style={{ border: 'none', padding: '10px', borderRadius: '4px', display: 'flex', alignItems: 'center', marginRight: '20px', backgroundColor: 'transparent' }}>
                <FontAwesomeIcon icon={faRedo} style={{ color: 'orange' }} size="2x" />
              </button>
              <button
              onClick={handleCalculate}
              disabled={isInputEmpty} // Disable the button if input is empty
              style={{
                borderRadius: '4px',
                ...
                border: 'none',
                boxShadow: 'none',
                cursor: isInputEmpty ? 'not-allowed' : 'pointer' // Change cursor based on input
              }}
            >
              Calculate
            </button>
            ...
            <pre style={{ fontWeight: 'bold', textAlign: 'center', paddingBottom: '20px' }}>{text}</pre>
            {showTable && ( <div ref={dimensionsRef}>
          <TableContainer>
